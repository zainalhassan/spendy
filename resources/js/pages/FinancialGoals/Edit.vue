              ...
              <Card class="bg-gray-50 dark:bg-slate-700 border-0">
                <template #content>
                  <div class="p-6">
                    ...
                    <small class="text-gray-600 dark:text-gray-300">
                      Inactive goals won't appear in your main dashboard
                    </small>
                  </div>
                </template>
              </Card>
...
onMounted(() => {
  ...
  form.currency_id = props.goal.currency_id
  form.year = props.goal.year
  form.start_amount = props.goal.start_amount
  form.target_amount = props.goal.target_amount
  form.expected_amount = props.goal.expected_amount
  form.is_active = props.goal.is_active
})

const submitForm = () => {
  loading.value = true
  
  form.put(`/financial-goals/${props.goal.id}`, {
    onSuccess: () => {
      loading.value = false
    },
    onError: () => {
